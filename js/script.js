...
function updateClock() {
    ...
    document.getElementById('clock').innerText = `${formattedHours}:${formattedMinutes} ${ampm}`;
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to update taskbar buttons
    function updateTaskbar() {
        taskbarPrograms.innerHTML = ''; // Clear existing buttons

        const windows = document.querySelectorAll('.window');
        windows.forEach((windowElement, index) => {
            // Create a button for each window
            const button = document.createElement('button');
            button.innerText = windowElement.querySelector('.title-bar-text').innerText || `Window ${index + 1}`;
            button.className = 'taskbar-button';

            // Handle button click: toggle minimize/restore
    ...
    // Close button logic
    document.querySelectorAll('[aria-label="Close"]').forEach(closeButton => {
    ...
    // Make windows draggable and handle z-index
    $(".window").draggable({
        start: function () {
            maxZIndex++;
            $(this).css('z-index', maxZIndex);
...
linkButton.addEventListener("click", function () {
    ...
    window.open("https://badriyahs.github.io/Thesis/", "_blank");
});